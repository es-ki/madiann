import { fetchArticles, fetchCategories } from '$lib/functions/microcms';
import { PUBLIC_SITE_URL } from '$env/static/public';

export async function GET() {
	const siteUrl = PUBLIC_SITE_URL || '';
	const [{ contents: categories }, { contents: articles }] = await Promise.all([
		fetchCategories({ limit: 999 }),
		fetchArticles({ limit: 999 })
	]);
	const categoryPaths = categories.map(({ id, publishedAt }) => ({
		path: id,
		lastMod: publishedAt
	}));
	const articlePaths = articles.map(({ id, category, updatedAt }) => ({
		path: `${category.id}/${id}`,
		lastMod: updatedAt
	}));
